...
;  Global Symbols
;-----------------------------------------------
export     I2CHW_1_RsrcStatus                                        ; upper (7x) bits write status, lower 3 bits read status
export    _I2CHW_1_RsrcStatus                                        ; upper (7x) bits write status, lower 3 bits read status
...
I2CHW_1_Poll_Status:                             blk      1  ;internal variable used for polled operation
...
;       care of it.
    ;case 1
    RAM_SETPAGE_CUR	( >I2CHW_1_Poll_Status )

    tst [I2CHW_1_Poll_Status], I2C_WRITE_STARTED
    jz  NOT_I2C_Write
    tst reg[ I2CHW_1_SCR], I2C_STOP_ST
    jz  NOT_I2C_Write
...
NOT_I2C_Write:    
    ...
    jnz  I2CHW_1_Process   ;if no other critical eventa are in evidence, process an I2C byte if it's done
    ret

 I2CHW_1_Process:
_I2CHW_1_Process:  ;don't need to export user will call I2CA_POLL which will call the I2CA_Process routine if necessary
...
; Check the Stop Bit in SCR only if the Stop Interrupt is enabled.    
    ...
    M8C_SetBank1
    tst reg[I2CHW_1_CFG],I2C_STOPIE
    M8C_SetBank0
    ELSE
    tst reg[I2CHW_1_CFG],I2C_STOPIE
    ENDIF
    jz  ByteComplProcess
    tst reg[I2CHW_1_SCR],I2C_STOP_ST                         ; Test if the Interrupt is due to Stop Condition
...
;* Slave only
    ...
    tst reg[I2CHW_1_SCR], I2C_TX  ;if set we are transmitting => this is at end of byte to eval the ACK
...
;********************************************************
   ...
   jc    I2CHW_1_WriteOverflow                                       ; carry set if value became -1
   ;jz    WrStoreData
   cmp   [I2CHW_1_Write_Count], 00                                   ;set nak flag, dec count, and store data
   jz    WrNakNextByte
   jmp   WrNotBufEnd
WrNakNextByte:                                                       ;set the nakflag in I2CHW_1_bStatus
...
;
I2CHW_1_WriteOverflow:
   or    [I2CHW_1_RsrcStatus], I2CHW_WR_OVERFLOW                     ; flag the overflow
   and   [I2CHW_1_RsrcStatus], ~I2CHW_WR_NOERR
   inc   [I2CHW_1_Write_Count]                                       ; put it back to zero
...
ENDIF
    ...
    pop A
    RAM_RESTORE_NATIVE_PAGING      		;has no effect if this is an ISR
    I2CHW_1_SERVICE_RETURN                                                              ;return and wait for the next interrupt (on data)
...
I2C_AckAddr_RXdata:
IF I2CHW_1_POLLED_PROCESS
    or [I2CHW_1_Poll_Status], I2C_WRITE_STARTED
ELSE
    BitSetI2CHW_1_CFG ( I2C_STOPIE )
ENDIF
...
;********************************************************
I2C_EvaluateADDRIN:
    mov A, [I2CHW_1_RsrcStatus]
    and A, (I2CHW_WR_NOERR | I2CHW_WR_OVERFLOW)
    jz  NewADDRNotSet                                                                                             ;set new addr only if previous data appears to be in buffer
    or  [I2CHW_1_RsrcStatus], I2CHW_WR_COMPLETE                      ;Same bit as I2CHW_ISR_NEW_ADDR

IF I2CHW_1_POLLED_PROCESS						                              ;if a new addr was received it's OK to terminate stop detection
    and [I2CHW_1_Poll_Status], ~I2C_WRITE_STARTED            ;used to determine the end of a Master write to a slave.
ELSE			
    BitClrI2CHW_1_CFG I2C_STOPIE
ENDIF
...
IF (I2CHW_1_AUTO_ADDR_CHECK^1)   ;; for CY8C28X45 chip: skip address comparison and NACK sending stage-hardware will do this for us  if AutoAddressCompare feature is enabled.
                                          ;; The code in this pre-compiler directive will be executed for all chips except CY8C28X45.
    xor A,  I2CHW_1_SLAVE_ADDR                                       ;for an equate
    jnz I2C_Terminate
ENDIF
...
I2CHW_1_send_ack:                                                    ;slave send adress ack
...
;********************************************************
    ...
    and [I2CHW_1_RsrcStatus], ~I2CHW_RD_COMPLETE                     ;Master will NAK us at the end of transaction
...
FlashRdOverflow:
   ...
   and   [I2CHW_1_RsrcStatus], ~I2CHW_RD_NOERR
   ;set count back to 0
...
;;  CONTROL MACRO- writes to the SCR register and accounts for clock speed adjustments if necessary
;
    SetI2CHW_1_SCR A     ;set read bit as (bit0) and Addr bit (bit7)in the I2C_SCR
...
I2C_LastByteToMstr:
    or  [I2CHW_1_RsrcStatus], I2CHW_RD_NOERR                         ;Master NAK'ed us at the end of transaction
...
;;  CONTROL MACRO- writes to the SCR register and accounts for clock speed adjustments if necessary
;
    SetI2CHW_1_SCR 0     ;the default mode to wait for an address
...
;
    or  [I2CHW_1_RsrcStatus], I2CHW_WR_COMPLETE                      ;only SET by ISR USER must clear.
    and [I2CHW_1_RsrcStatus], ~I2CHW_ISR_ACTIVE

IF I2CHW_1_POLLED_PROCESS
    and [I2CHW_1_Poll_Status], ~I2C_WRITE_STARTED
    jmp ExitISR
ELSE
    BitClrI2CHW_1_CFG I2C_STOPIE
    tst reg[I2CHW_1_SCR], I2C_BYTE_COMPL
